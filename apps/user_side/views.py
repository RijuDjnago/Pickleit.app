...
def user_signup(request):
    context = {"map_key": settings.MAP_API_KEY}
    try:
        if request.method == "POST":
            username = request.POST.get("email", "").strip()
            password = request.POST.get("password", "").strip()
            ...
            user = User(username=username, email=email, password=password, 
                                    permanent_location=location, rank=rank, first_name=first_name, last_name=last_name,
                                    latitude=latitude if latitude not in ["", "null", None] else None, longitude=longitude if longitude not in ["", "null", None] else None)
            user.set_password(password)
            player = Player(player=user, player_first_name=first_name, player_last_name=last_name, player_email=email, player_ranking=rank)
            player.save()
            user.save()
            user = authenticate(request, username=username, password=password)
            if user is None:
        ...
        context['message'] = "An error occurred during signup. Please try again."
    return render(request, 'auth/signup.html', context)
...
def index(request):
    seen_names = set()
    events = []
    for event in Leagues.objects.exclude(team_type__name="Open-team").order_by('name'):
        if event.name not in seen_names:
            ...
            events.append({
                ...
                'image_url': event.image.url if event.image else 'https://static.vecteezy.com/system/resources/previews/014/814/180/non_2x/a-well-designed-flat-icon-of-no-event-vector.jpg',
                'thumbnail_color': '#ffa5009e'
            })
            if len(events) >= 3:  # Limit to 3 distinct events
                break
...
def user_list(request):
    ...
    # Pagination
    paginator = Paginator(user_data, 60)  # Show 12 users per page
    try:
...
def find_team_list(request):
    ...
    paginator = Paginator(teams, 61)  # Show 10 teams per page
...
def match_history(request):
    ...
    # Calculate wins and losses for all matches (unchanged by filters)
    match_history_cal = match_history.only("team1", "team2", "winner_team")
    

    # Get all leagues where the user's teams are registered
    user_leagues = Leagues.objects.filter(registered_team__in=teams).distinct()
    print(user_leagues, )
    # Apply league filter if selected
    if league_id:
        match_history = match_history.filter(leagues__id=league_id)
    ...
    total_matches = match_history.count()
    wins = sum(1 for match_ in match_history if match_.winner_team in teams)
