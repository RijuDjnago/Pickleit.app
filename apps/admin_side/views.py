...
def player_list_(request):
    ...
    if search_text:
       user_q = User.objects.filter(Q(first_name__icontains=search_text) |Q(last_name__icontains=search_text) |Q(username__icontains=search_text) |Q(email__icontains=search_text) |Q(rank__icontains=search_text) |Q(gender__icontains=search_text))
    if select_plan:
        if select_plan != "Free":
            # print(select_plan)
            ...
            user_ids = list(Subscription.objects.exclude(plan__name__in=['Paid with upgrade', 'Pro', 'Enterprise']).values_list("user_id", flat=True))
    ...
    users = user_q.values(
        ...
        "image", "rank", "gender", "email", "created_at"
    )
...
def player_view(request, user_id):
    ...
    transaction = WalletTransaction.objects.filter(Q(sender=user) | Q(reciver=user))
    all_buy_plan = Subscription.objects.filter(user=user)
    created_teams = Team.objects.filter(created_by=user)
    player = Player.objects.filter(player=user).prefetch_related('team')
    ...
    for team in teams:
        ...
        match_history = Tournament.objects.filter(Q(team1=team) | Q(team2=team)).distinct()
        for tour in match_history:
            if tour.winner_team == tour.team1:
                team1_score = tour.winner_team_score
                team2_score = tour.loser_team_score
            else:
                team1_score = tour.loser_team_score
                team2_score = tour.winner_team_score

            match_history_data = {
                "league_name":tour.leagues.name,
                "team1":tour.team1,
                "team2":tour.team2,
                "is_win": tour.winner_team==team,
                "team1_score":team1_score,
                "team2_score":team2_score,
                "match_number":tour.match_number,
                "is_drow": tour.is_drow,
                "winner": tour.winner_team,
            }
            all_match_history.append(match_history_data)
    
    context["user"] = user
    context["Wallet_details"] = Get_Wallet_details
    context["transaction"] = transaction
    context["all_team_data"] = all_team_data
    context["all_match_history"] = all_match_history
...
### tournamnet section end

#payment list
@login_required(login_url="/admin/login/")
def payment_table(request):
    context = {"message":""}
    add_fund_payment = AllPaymentsTable.objects.all()
    context["add_fund_payment"] = add_fund_payment
...
def social_feed_list(request):
    ...
    paginator = Paginator(feed_query, 10)  # Show 10 posts per page
    page_number = request.GET.get("page")
    feedlist = paginator.get_page(page_number)
...
def edit_social_feed(request, post_id):
    post = get_object_or_404(socialFeed, id=post_id, user=request.user)
...
def club_list(request):
    ...
    if lat and lng:
        ...
        max_distance_km = 100
        # Filter by distance (Haversine formula using geopy)
        filtered_clubs = []
        for club in clubs:
            club_location = (float(club.latitude), float(club.longitude))
            distance = geodesic(user_location, club_location).km
            if distance <= max_distance_km:
                filtered_clubs.append(club)
    else:
        filtered_clubs = clubs
    
    paginator = Paginator(filtered_clubs, 10)
    page = request.GET.get('page')
    try:
        clubs_paginated = paginator.page(page)
    except PageNotAnInteger:
        clubs_paginated = paginator.page(1)
    except EmptyPage:
        clubs_paginated = paginator.page(paginator.num_pages)
    
    for club_ins in clubs_paginated:
        ...
        if find_image:
            ...
            club_ins.image = None
    google_maps_api_key = "AIzaSyAfBo6-cZlOpKGrD1ZYwISIGjYvhH_wPmk"
    context = {
        'clubs': clubs_paginated,
        'keyword': keyword,
        'location':location,
        'lat': lat,
        'lng': lng,
