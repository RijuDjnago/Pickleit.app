            ...
            if variation_type != 'single':
                ...
                for var_id in set(variation_ids):
                    ...
                    if key in seen_combinations:
                        ...
                        return render(request, 'dashboard/side/add_product_form.html')
    ...
    if request.method == "POST":
        ...
        if variation_type != 'single':
            ...
            for var_id in set(variation_ids):
                ...
                if key in seen_combinations:
                    messages.error(request, f"Duplicate specification detected: size/color combination '{key}' must be unique.")
                    return render(request, 'dashboard/side/edit_product_form.html')
...
def my_received_orders(request):    
    ...
    return render(request, 'dashboard/side/my_orders.html',
...
def my_placed_orders(request):
    ...
    try:
        ...
        order_page = paginator.page(1)
    return render(request, 'dashboard/side/my_placed_orders.html',
                   {'orders': order_page,
                    "page_range": get_custom_pagination_range(order_page, paginator),
                    "search_text": search_text,
                    "page_length": str(page_length)
...
def add_to_cart(request):
    ...
    try:
        product = MerchandiseStoreProduct.objects.get(id=product_id)
        spec = MerchandiseProductSpecification.objects.get(product=product, size=size, color=color)
...
def checkout_summary(request, order_id):
    try:
        ...
        return render(request, 'dashboard/side/checkout_summary.html', context)

    except CustomerMerchandiseStoreProductBuy.DoesNotExist:
        return redirect('dashboard:store_home')
...
def wishlist(request):
    ...
    return render(request, "dashboard/side/wishlist.html", context)
...
def cart(request):
    ...
    return render(request, 'dashboard/side/cart.html', context)
...
def remove_cart_item(request):
    if request.method == "POST":
        try:
            ...
            # Delete all items matching this group
            CustomerMerchandiseStoreProductBuy.objects.filter(
                created_by=request.user,
...
def select_address(request):
    ...
    return render(request, 'dashboard/side/select_address.html', {
        'addresses': addresses,
...
def select_address_buy_now(request):
    ...
    return render(request, 'dashboard/side/select_address_buy_now.html', {
        'addresses': addresses,
...
def cart_payment_gateway(request):
    ...
    return render(request, 'dashboard/side/cart_payment_gateway.html', {
...
def cart_payment_success(request):
    ...
    if not session_id:
        return render(request, "dashboard/side/payment_cancel.html", {"error": "Invalid session."})
    ...
    try:
        ...
        if session.payment_status == "paid":
            ...
            return render(request, "dashboard/side/payment_success.html", {"amount": amount})
        else:
            return render(request, "dashboard/side/payment_cancel.html", {"error": "Payment not completed."})
    except stripe.error.StripeError as e:
        return render(request, "dashboard/side/payment_cancel.html", {"error": str(e)})


@superuser_required
def cart_payment_cancel(request):
    return render(request, "dashboard/side/payment_cancel.html", {"error": "Payment was cancelled."})
...
def buy_now_payment_gateway(request):
    ...
    print(order_id)
    return render(request, 'dashboard/side/buy_now_payment_gateway.html', {
        'total_amount': total_amount,
        'order_id': order.id,
        'STRIPE_PUBLISHABLE_KEY': settings.STRIPE_PUBLIC_KEY
    })
...
def buy_now_payment_success(request):
    ...
    if not session_id or not order_id:
        return render(request, "dashboard/side/payment_cancel.html", {"error": "Missing session or order ID."})
...
def get_transaction_details(request, transaction_id):
    ...
    data = {
        ...
        "transaction": {
            ...
            'receiver':transaction.reciver.username,
            "amount": float(transaction.amount),
            'receiver_cost': float(transaction.reciver_cost),
            "admin_cost": float(transaction.admin_cost),
