...
def index(request):
    ...
    log_entries = LogEntry.objects.order_by('-timestamp')[:20]
...
def payment_table(request):
    context = {"message":""}
    payments_for_buy_product = PaymentTable.objects.filter(payment_for="product_buy")
    payment_for_advertisemnet = PaymentTable.objects.all().exclude(payment_for="product_buy")
