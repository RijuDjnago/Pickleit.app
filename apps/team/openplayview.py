...
def create_open_play_event_randomize(request):
    ...
    try:
        ...
        try:
            leagues_start_datetime = datetime.strptime(leagues_start_date, '%Y-%m-%d %H:%M:%S')
        ...
        # Send notifications
        title = "Create Open Play event invitation"
...
def create_selected_user_team(request):
    ...
    try:
        ...
        if event.play_type != 'Robbin Randomizer':
            raise ValidationError("Event must have 'Robbin Randomizer' play type")
