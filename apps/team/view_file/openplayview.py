...
def create_selected_user_team(request):
    ...
    try:
        ...
        for player_set in user_sets:
            ...
            if event.play_type in ["Round Robin", "Round Robin Compete to Final", "Robin Randomizer"]:
                from apps.team.view_file.eventview import handle_round_robin
                registered_teams = event.registered_team.all()
                team_ids = [team.id for team in registered_teams]
                play_details = LeaguesPlayType.objects.filter(league_for=event).values("data").first()
                ...
                handle_round_robin(data, event, team_ids, round_robin, event.play_type)
        
        data['message'] = "Teams and matches created successfully"
