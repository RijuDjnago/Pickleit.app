...
class Player(models.Model):
    ...
    player_phone_number = PhoneNumberField(null = True, blank = True)
...
class Leagues(models.Model):
    ...
    registered_team = models.ManyToManyField(Team)
...
class TournamentSetsResult(models.Model):
    ...
    def __str__(self):
        ...
        try:
            ...
            team_2 = None
        return f"{self.tournament.leagues.name} ({team_1} VS {team_2}) || Match Number {self.tournament.match_number} || Point {self.team1_point}-{self.team2_point} || win {self.win_team}"
        #return f"{self.tournament.leagues.name} ({self.tournament.team1.name} vs {self.tournament.team2.name}) || Match Number {self.tournament.match_number} || Point {self.team1_point}-{self.team2_point} || win {self.win_team}"
