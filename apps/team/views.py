    ...
    # try:
        ...
        if check_user.exists() and check_leagues.exists():
            ...
            for sc in tournament_details:
                if sc["group__court"] is None:
                    sc["group__court"] = sc["court_sn"]
                team_1_plater = list(Player.objects.filter(team__id=sc["team1_id"]).values_list("player_id", flat=True))
                team_2_plater = list(Player.objects.filter(team__id=sc["team2_id"]).values_list("player_id", flat=True))
                if get_user == league.created_by or get_user.id in sub_org_list or sc["team1_id"] == get_user.id or sc["team2_id"] == get_user.id:
                    sc["is_edit"] = True
                else:
                    sc["is_edit"] = False
                test.append({"tt":team_1_plater,"ttt":team_2_plater,"s":sc["is_edit"]})
                if sc["team1__team_image"] != "":
                    img_str = sc["team1__team_image"]
                    sc["team1__team_image"] = f"{media_base_url}{img_str}"
                if sc["team2__team_image"] != "":
            ...
            if check_player.exists():
                ...
                if len(team_ids) > 0:
                    ...
                    for team_id in team_ids:                        
                        ...
                        check_match = Tournament.objects.filter(Q(team1_id=team_id, is_completed=True) | Q(team2_id=team_id, is_completed=True))
        ...
        if check_user:
            ...
            if check_player:
                ...
                joined = list(Leagues.objects.filter(
                                    Q(registered_team__in=player_teams,leagues_end_date__gte=today,is_complete=False) | 
...
def my_team_list(request):
    ...
    try:    
        ...
        if check_user.exists():
            ...
            paginator = PageNumberPagination()
            paginator.page_size = 20
            paginated_teams = paginator.paginate_queryset(my_team, request)
            paginated_response = paginator.get_paginated_response(paginated_teams)
