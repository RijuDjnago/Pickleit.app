...
def block_or_unblock_chat_user(request):
    try:
        ...
        if not status_field:
            data["status"], data["message"] = status.HTTP_400_BAD_REQUEST, "Status field is required"

        status_true = status_field.lower() in ['true', '1']
        
        if status_field.lower() == 'false':
            status_true = False
        check_user = User.objects.filter(uuid=user_uuid, secret_key=user_secret_key)
        check_block_user = User.objects.filter(uuid=block_user_uuid, secret_key=block_user_secret_key)
        if check_user.exists() and check_block_user.exists():
            get_user = check_user.first()
            get_block_user = check_block_user.first()
            check_room = Room.objects.filter(Q(user_one=get_user, user_two=get_block_user) | Q(user_one=get_block_user, user_two=get_user))
            if check_room.exists():
                get_room = check_room.first()
            else:
            ...
            if not status_true:
                if u1 == get_user:
                    get_room.is_blocked_user_two = False
                    get_room.save()
                if u2 == get_user:
                    get_room.is_blocked_user_one = False
                    get_room.save()
                data["status"], data["message"] = status.HTTP_200_OK, "User unblocked successfully"
            else:
                if u1 == get_user:
                ...
                if u2 == get_user:
                    ...
                    get_room.save()
                data["status"], data["message"] = status.HTTP_200_OK, "User blocked successfully"
        else:
            data['status'] = status.HTTP_401_UNAUTHORIZED
            data['message'] = "Unauthorized access"
