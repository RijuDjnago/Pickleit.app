...
# class ChatConsumerAdmin(AsyncWebsocketConsumer):
...
#         self.get_check_sender = await self.get_user(self.sender_id)
#         self.get_room = await self.get_room_by_name(self.room_name)
        
#         if self.get_check_sender and self.get_room:
#             self.get_reciver_user = self.get_room.user_two if self.get_check_sender == self.get_room.user_one else self.get_room.user_one

#             await self.set_cache(self.room_name, self.room_name)
#             await self.set_cache(self.sender_id, self.sender_id)
...
#         all_messages = await self.get_unread_messages(self.get_room, self.get_check_sender)
#         online_user = await self.get_online_users()
#         block_status, message = await self.check_block_status()
#         first_time_status = not await self.has_previous_messages(self.get_room, self.get_check_sender)

#         await self.send(text_data=json.dumps({
#             "message": message, "get_datetime": "", "online_user": online_user,
#             "block_status": block_status, "first_time_chat": first_time_status
#         }))
        
#         for msg in all_messages:
#             await self.send(text_data=json.dumps({
#                 "message": msg.text_message, "get_datetime": str(msg.created_at),
#                 "block_status": block_status, "first_time_chat": first_time_status
#             }))
#             await self.mark_message_as_read(msg)

#         serializer = MessageBoxSerializer(all_messages, many=True)
#         await self.send(text_data=json.dumps({
#             "messages": serializer.data, "block_status": block_status,
#             "first_time_chat": first_time_status, "online_user": online_user
#         }))
    
#     async def disconnect(self, close_code):
#         await self.delete_cache(self.sender_id)
#         await self.delete_cache(self.get_reciver_user.id)
#         online_user = await self.get_online_users()
#         await self.send(text_data=json.dumps({"message": "", "get_datetime": "", "online_user": online_user}))
...
#     @database_sync_to_async
#     def get_unread_messages(self, room, reciver_user):
#         return MessageBox.objects.filter(room=room, reciver_user=reciver_user, is_read=False)

#     @database_sync_to_async
#     def has_previous_messages(self, room, sender_user):
#         return MessageBox.objects.filter(room=room, sender_user=sender_user).exists()
...
#         if self.get_room.is_blocked_user_one and self.get_room.is_blocked_user_two:
#             block_status = True
#             if self.get_check_sender == self.get_room.user_one:
#                 message = f"You have blocked {self.get_room.user_two.first_name} and they have blocked you too."
#             else:
#                 message = f"You have blocked {self.get_room.user_one.first_name} and they have blocked you too."
#         elif self.get_room.is_blocked_user_one or self.get_room.is_blocked_user_two:
#             block_status = True
#             if self.get_room.is_blocked_user_one:
#                 if self.get_check_sender == self.get_room.user_one:
#                     message = f"{self.get_room.user_two.first_name} has blocked you."
#                 else:
#                     message = f"You have blocked {self.get_room.user_one.first_name}."
#             if self.get_room.is_blocked_user_two:
#                 if self.get_check_sender == self.get_room.user_one:
#                     message = f"You have blocked {self.get_room.user_two.first_name}."
#                 else:
#                     message = f"{self.get_room.user_one.first_name} has blocked you."
#         return block_status, message

#     @database_sync_to_async
#     def get_online_users(self):
#         online_user = []
#         if cache.get(str(self.sender_id)):
#             online_user.append(int(self.sender_id))
#         if cache.get(str(self.get_reciver_user.id)):
#             online_user.append(int(self.get_reciver_user.id))
#         return online_user
...
#                 room=self.get_room, sender_user=self.get_check_sender,
#                 reciver_user=self.get_reciver_user, is_read=False, text_message=message
#             )

#     @database_sync_to_async
#     def get_serialized_messages(self):
#         all_messages = MessageBox.objects.filter(room=self.get_room).filter(Q(text_message__isnull=False) & ~Q(text_message=" "))
#         return MessageBoxSerializer(all_messages, many=True)

#     @database_sync_to_async
#     def filter_bad_words(self, message):
#         if message:
#             message_list = message.split(" ")
#             new_message = " ".join(["****" if ch.lower() in bad_words_list else ch for ch in message_list])
#             return new_message
#         return message
